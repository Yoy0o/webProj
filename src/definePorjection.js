(function definePorjection() {
	var forEachPoint = function(func) {
		...
		/**
		 * 知道了这个算法之后，就可以离线进行 Google 地图偏移校正
		 * 地形图非线性保密处理技术
		 */
		var gcj02 = {};
		var PI = Math.PI;
		...
		var baiduMercatorProj = new ol.proj.Projection({
			code: 'BD09',
			extent: ol.extent.applyTransform([72.004, 0.8293, 137.8347, 55.8271], projzh.ll2bmerc),
			units: 'm'
		});
		...
		// 高德坐标偏移定义
		var mc2gcj02mc = function(input, opt_output, opt_dimension) {
		  var output = sphericalMercator.inverse(input, opt_output, opt_dimension);
		  output = gcj02.fromWGS84(output, output, opt_dimension);
		  return sphericalMercator.forward(output, output, opt_dimension);
		}

		var gcj02mc2mc = function(input, opt_output, opt_dimension) {
		  var output = sphericalMercator.inverse(input, opt_output, opt_dimension);
		  output = gcj02.toWGS84(output, output, opt_dimension);
		  return sphericalMercator.forward(output, output, opt_dimension);
		}

		var gcj02mc2ll = function(input, opt_output, opt_dimension) {
		  var output = sphericalMercator.inverse(input, opt_output, opt_dimension);
		  return gcj02.toWGS84(output, output, opt_dimension);
		}

		var ll2gcj02mc = function(input, opt_output, opt_dimension) {
		  var output = gcj02.fromWGS84(input, opt_output, opt_dimension);
		  return sphericalMercator.forward(output, output, opt_dimension);
		}

		var gcj02mc = new ol.proj.Projection({
		  code: 'GCJ02',
		  extent: ol.extent.applyTransform([-180, -90, 180, 90], ll2gcj02mc),
		  units: 'm'
		});

		ol.proj.addProjection(gcj02mc);
		ol.proj.addCoordinateTransforms('EPSG:4326', gcj02mc, ll2gcj02mc, gcj02mc2ll);
		ol.proj.addCoordinateTransforms('EPSG:3857', gcj02mc, mc2gcj02mc, gcj02mc2mc);
})();
